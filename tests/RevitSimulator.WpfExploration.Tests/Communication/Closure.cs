...
namespace RevitSimulator.WpfExploration.Tests.Communication.Closure;
...
{
    [Fact]
    public void ShouldTrackProgressUsingClosureGeneratedCallback()
    {
        ...
        var createOnRoomComputed = (int totalCount) =>
        {
            var completedCount = 0;
            OnRoomAreaComputed onRoomComputed = (room, factor, computedArea) =>
            {
                completedCount++;
            ...
            return onRoomComputed;
        };

        var onRoomComputed = createOnRoomComputed(rooms.Count);

        var sum = dependency.ComputeAreas(rooms, onRoomComputed);
